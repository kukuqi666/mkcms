...
<li><input type="submit" value="提交" name="save" class="btn btn-primary pull-right" />
<input type="reset" value="重填" class="btn btn-default" />
</li></ul></div></form></div>
</div></div>
<div class="col-lg-wide-7 col-xs-1">
<?php
$sql = 'select * from mkcms_book order by id asc';
...
while($row= mysqli_fetch_array($result))
	{
$zhStr = $row['Reply'];
echo '<div class="stui-pannel stui-pannel-bg clearfix"><div class="stui-pannel-box clearfix"><div class="col-pd clearfix"><ul><li class="topwords"><strong>',$row['userid'],'</strong><span class="text-red pull-right">第',$i,'楼</span></li><li class="top-line" style="margin-top: 10px; padding: 10px 0;">',$row['content'],'<br/><font color=red>';
if(0 < strlen($zhStr)){echo '管理员回复：',$row['Reply'];}echo '</font>
</li><span class="font-12 text-muted">发表于: ',$row['time'],'</span></ul></div></div></div>';
$i++;}
echo '<ul class="stui-page text-center">',page_show1($pager[2],$pager[3],$pager[4],2),'</ul>
</div>
</div>
</div>
</div>';?>
