...
alert_href('您现在所属会员组的权限制大于等于目标会员组权限值，不需要升级!','mingxi.html');
}
...
alert_href('续费成功!','mingxi.html');
}

}
	else{
alert_href('您的积分不够，无法续费,请继续赚取积分或其他方式购买会员!','mingxi.html');
}
}
else{//普通会员充值
...
$sql = 'update mkcms_user set '.arrtoupdate($_data).' where u_name="'.$_SESSION['user_name'].'"';
if (mysqli_query($conn,$sql)) {
alert_href('升级成功!','mingxi.html');
}

}
	else{
alert_href('您的积分不够，无法升级到该会员组,请充值!','mingxi.html');
}	
}
}
}
